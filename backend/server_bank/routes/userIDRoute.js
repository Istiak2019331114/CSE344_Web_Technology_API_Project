...
const User = require('../models/userIDModel.js');

// Bank ID registration
router.post("/register",async(req,res)=>{

    const {bankUID ,email, password ,bdt} = req.body;
    ...
    try{
        ...
        const newUser = new User({bankUID,email,password:hashedPassword ,bdt});
        newUser.save();
...
// Checking if a user has a bank account upon loging in basis of email
...
// Admin updating  balance after a purchase 
router.post("/updateAdminBalance",async(req,res)=>{
    ...
    try {
        ...
        await User.updateOne(
            { 'email' : 'supplier@gmail.com' },{ $inc: { 'bdt': amount } }
            );
...
router.post("/updateBalance",async(req,res)=>{
    ...
    try {
        ...
        await User.updateOne(
            { 'email' : 'admin@gmail.com' },{ $inc: { 'bdt': amount } }
        );
